...
async function tenantId(){
  if(!tenantIdPromise){
    tenantIdPromise=db.from('tenants').select('id').eq('slug',process.env.DEFAULT_TENANT_SLUG||'noun').eq('status','active').single()
      .then(({data,error})=>{if(error||!data)throw error||new Error('Active tenant missing');return data.id;});
  }
...
async function recordDemand({category,query,course,level,phone}){const tid=await tenantId(); const {error}=await db.from('intelligence_signals').insert({tenant_id:tid,signal_type:'service_demand',category,query:query||null,course:course||null,level:level||null,phone_hash:phone?require('crypto').createHash('sha256').update(phone).digest('hex'):null,created_at:new Date().toISOString(),metadata:{privacy:'phone hashed; use only for aggregate intelligence'}}); if(error)throw error; return true;}

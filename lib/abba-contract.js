const ABBA_PROTOCOL_VERSION = '1.1';
...
function buildAbbaResult(result = {}) {
  return {
    ...
    context: result.context && typeof result.context === 'object' ? result.context : null,
    escalation: result.escalation && typeof result.escalation === 'object' ? result.escalation : null,
    opportunityMatches: Array.isArray(result.opportunityMatches) ? result.opportunityMatches : [],
    pulse: result.pulse && typeof result.pulse === 'object' ? result.pulse : null,
    event: result.event && typeof result.event === 'object' ? result.event : null,
    evidenceDecision: result.evidenceDecision || null,

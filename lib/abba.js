const {db,searchKnowledge,buildGrounding,tenantId}=require('./knowledge');

const EVENT_TYPES=new Set([
  'news','academic_change','policy_change','course_change','programme_change','student_impact','service_opportunity','internship','event','student_life','skills','employment','apprenticeship','nysc','student_business','student_governance','insight_candidate','campaign_candidate'
]);
...
function classifyEvent(x={}){
  ...
  if(/ballot|election|vote|student government|student union/.test(s))return 'student_governance';
  if(/nysc/.test(s))return 'nysc';
  if(/apprentice|apprenticeship/.test(s))return 'apprenticeship';
  if(/internship|siwes|job|employment|vacanc/.test(s))return /internship|siwes/.test(s)?'internship':'employment';
  if(/skill acquisition|skills acquisition|training|workshop|bootcamp|masterclass/.test(s))return 'skills';
  if(/sport|football|basketball|athletic|excursion|tour|club|association|matric|graduat|gown|regalia/.test(s))return 'student_life';
  if(/textbook|handout|course material|printing|photocopy|binding|signature|signing|id card|clearance|transport|accommodation/.test(s))return 'service_opportunity';
  ...
  if(/student business|marketplace|vendor|service provider/.test(s))return 'student_business';
...
function decide({event_type,authority_tier=4,confidence=0,student_impact=false,source_verified=false}={}){
  ...
  if(['policy_change','academic_change','course_change','programme_change','student_governance'].includes(event_type))return {action:'recommend',status:'ready_for_review',reason:'Institutional or governance change requires impact review before broadcast.'};
...
async function studentBrief(phone,query){
  const tid=await tenantId();
  const {data:student,error}=await db.from('students').select('phone,name,level,courses,faculty,department,email').eq('tenant_id',tid).eq('phone',phone).maybeSingle();

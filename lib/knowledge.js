...
const kinds=['claim','policy','course','programme','event','assessment','source'];const facts=[];for(const [i,r] of results.entries())for(const x of r.data||[]){const kind=kinds[i];facts.push({...x,kind,score:rank(x,q)})}facts.sort((a,b)=>b.score-a.score);const top=facts.slice(0,opts.limit||10);return {facts:top,sources:top.map(x=>({tier:x.authority_tier||x.source_tier||4,title:x.title||x.claim||x.question_text||x.name,url:x.source_url||x.url||null})),confidence:top.some(x=>x.score>.62)?'high':top.length?'medium':'low'}}
...
function buildStudentGrounding({knowledge={facts:[],sources:[],confidence:'low'},courseContent=null}={}){
  ...
  const sources=[...(knowledge.sources||[]),...(courseContent?[{tier:2,title:'Matched course content',url:null}]:[])];

...
const missingDb = new Proxy({}, { get(){ throw new Error('Supabase is not configured'); } });
let db;
function getDb(){
  if(db)return db;
  if(!process.env.SUPABASE_URL || !process.env.SUPABASE_SERVICE_ROLE_KEY) return missingDb;
  db=createClient(normalizeSupabaseUrl(process.env.SUPABASE_URL),process.env.SUPABASE_SERVICE_ROLE_KEY,{auth:{persistSession:false}});return db;
}
const TIER_NAMES={1:'official_noun',2:'official_courseware',3:'official_news',4:'student_or_verified_secondary',5:'external'};
let tenantIdPromise;async function tenantId(){if(!tenantIdPromise)tenantIdPromise=getDb().from('tenants').select('id').eq('slug',process.env.DEFAULT_TENANT_SLUG||'noun').eq('status','active').single().then(({data,error})=>{if(error||!data)throw error||new Error('Active tenant missing');return data.id;});return tenantIdPromise;}
...
async function safe(table,select,filters=[]){const tid=await tenantId();let q=getDb().from(table).select(select).eq('tenant_id',tid);for(const f of filters)q=q[f[0]](f[1],f[2]);return q.limit(500)}
...
async function recordActivity(phone,event_type,course_code=null,topic=null,metadata={}){if(phone){const tid=await tenantId();await getDb().from('student_activity').insert({tenant_id:tid,phone,event_type,course_code,topic,metadata})}}
module.exports={get db(){return getDb()},searchKnowledge,buildGrounding,buildStudentGrounding,recordActivity,TIER_NAMES,tenantId};

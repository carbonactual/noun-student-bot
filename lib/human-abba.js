...
function handoffUrl({ requestId, name, message, stage, area, course, service }) {
  const lines = [
...
async function queueHumanAbba(input = {}) {
  ...
  return {
    ...
    route: 'service_request',
    channel: 'whatsapp',
    handoffUrl: handoffUrl({ requestId, name, message, stage, area, course, service: clean(input.service, 140) })
  };
}

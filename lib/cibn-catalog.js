...
const SOURCE = {
  ...
  OFFICIAL_TIMETABLE_RULE: 'cibn_official_october_2026_timetable',
  OFFICIAL_TIMETABLE_2026: 'cibn_official_october_2026_timetable_pdf',
...
function row(program, level, code, name, date, time, type, opts = {}) {
  return {
    ...
    note: opts.note || null,
    durationMinutes: Number(opts.durationMinutes || 90)
  };
}
...
const COURSE_MAP = [
  ...
  row('MCP', 'Microfinance I', 'MF301', 'The Evolution Management and Regulation of Microfinancing', '2026-10-06', '09:00-12:00', 'core', {source: SOURCE.OFFICIAL_TIMETABLE_2026, authority: 'official_cibn', durationMinutes: 90}),
  row('MCP', 'Microfinance I', 'MF302', 'Financial Analysis and Performance Monitoring in Microfinance Institutions', '2026-10-06', '09:00-12:00', 'core', {source: SOURCE.OFFICIAL_TIMETABLE_2026, authority: 'official_cibn', durationMinutes: 90}),
  row('MCP', 'Microfinance I', 'MF303', 'Product Development and Marketing Management', '2026-10-06', '14:00-17:00', 'core', {source: SOURCE.OFFICIAL_TIMETABLE_2026, authority: 'official_cibn', durationMinutes: 90}),
  row('MCP', 'Microfinance II', 'MF401', 'Risk Management and Internal Control in Microfinance Institutions', '2026-10-07', '09:00-12:00', 'core', {source: SOURCE.OFFICIAL_TIMETABLE_2026, authority: 'official_cibn', durationMinutes: 90}),
  row('MCP', 'Microfinance II', 'MF402', 'Ethics and Corporate Governance', '2026-10-06', '14:00-17:00', 'core', {source: SOURCE.OFFICIAL_TIMETABLE_2026, authority: 'official_cibn', durationMinutes: 90}),
  row('MCP', 'Microfinance II', 'MF403', 'Digital Finance in Microfinance Institution', '2026-10-07', '09:00-12:00', 'elective', {source: SOURCE.OFFICIAL_TIMETABLE_2026, authority: 'official_cibn', note: 'October 2026 timetable visual: Wednesday morning session; alternative to MF404.', durationMinutes: 90}),
  row('MCP', 'Microfinance II', 'MF404', 'Small and Medium Enterprises Management and Development', '2026-10-07', '14:00-17:00', 'elective', {source: SOURCE.OFFICIAL_TIMETABLE_2026, authority: 'official_cibn', note: 'October 2026 timetable visual: Wednesday afternoon session; alternative to MF403.', durationMinutes: 90}),
...
const SESSION_RULE = {
  rule: 'Candidates cannot combine courses exceeding the 3-hour duration at a particular session.',
...
function sessionGroups(codes) {
  ...
  return [...groups.entries()].map(([slot, items]) => ({
    ...
    totalDurationMinutes: items.reduce((sum, x) => sum + x.durationMinutes, 0),
    withinThreeHourCap: items.reduce((sum, x) => sum + x.durationMinutes, 0) <= 180
  }));
}

function sameSession(codes) {
  return sessionGroups(codes).filter(x => x.courses.length > 1 && x.withinThreeHourCap).map(x => ({
    ...x,
    status: 'same_session_within_cap'
  }));
}
...
function conflicts(codes) {
  return sessionGroups(codes).filter(x => x.courses.length > 1 && !x.withinThreeHourCap).map(x => ({
    ...x,
    status: 'session_duration_exceeded'
  }));
}

const RULES = {
  exemptionFailedSubject: {
    rule: 'A candidate cannot be exempted from a subject they attempted and failed.',
    source: SOURCE.PORTAL,
    authority: 'portal_observation'
  },
  exemptionTiming: {
    rule: 'Exemption must be completed before the candidate sits the last examination paper.',
    source: SOURCE.PORTAL,
    authority: 'portal_observation'
  },
  exemptionUpload: {
    rule: 'The owner portal evidence displayed an exemption upload requirement of one PDF not larger than 2MB.',
    source: SOURCE.PORTAL,
    authority: 'portal_observation'
  },
  oldSubjectMapping: {
    rule: 'The owner portal screenshot journey showed the old-subject-to-new-syllabus mapping link returning HTTP 404.',
    source: SOURCE.PORTAL,
    authority: 'portal_observation'
  }
};

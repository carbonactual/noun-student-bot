...
// configured. Uses GEMINI_API_KEY (Google AI Studio free tier).
// Resilience: short per-attempt timeout, one retry on 503/429, and a fallback
// model, so transient Google load does not take the student chat down.
// ---------------------------------------------------------------------------
...
const GEMINI_SYSTEM_PROMPT = [
  ...
  'Format: short paragraphs or a compact numbered list. Max ~180 words. End with one helpful next step.',
  'Output plain text only — no markdown. Never use ** or ## symbols. Emphasize key words by stating them plainly.',
  '',
  'HARD RULES:',
...
async function invokeGemini(request, dependencies = {}) {
  ...
  // Attempt 1: primary model. Retry once on transient Google load (503/429).
  for (let i = 0; i < 2; i++) {
    try {
      const text = await geminiAttempt(PRIMARY_MODEL, apiKey, request, dependencies, attemptMs);
      ...
      break; // aborts/404s fall through to the fallback model
    }
  }

  // Attempt 2: fallback model on any primary failure.
  try {
    const text = await geminiAttempt(FALLBACK_MODEL, apiKey, request, dependencies, attemptMs);
    return buildAbbaResult({
      requestId: request.requestId,
      capability: request.capability,
      answer: text,
    ...
    throw lastError && String(lastError.message).includes('aborted') ? lastError : error;
  }
}

...
const GEMINI_SYSTEM_PROMPT = [
  ...
  '5. Never claim to be human. You are ABBA, an AI companion.',\n  '6. Output plain text only — no markdown. Never use ** or ## symbols. Bold key words simply by stating them plainly or with *single asterisks*.'
].join('\n');
...
async function invokeGemini(request, dependencies = {}) {
  ...
  try {
    ...
    let response = await fetchImpl(url, fetchOptions);
    ...
    const text = data && data.candidates && data.candidates[0] && data.candidates[0].content
      ...
      : '';
    const cleanText = text.replace(/\*\*/g, '').replace(/^#+\s*/gm, '').trim();\n    if (!cleanText) throw new Error('Gemini engine returned an empty answer');

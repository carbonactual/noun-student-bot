...
async function cibnChat(req, res) {
  ...
  const attempts = [
    ...
    {model:FALLBACK_MODEL, timeout:3200},
    {model:process.env.CIBN_GEMINI_LAST_FALLBACK_MODEL || process.env.GEMINI_LAST_FALLBACK_MODEL || 'gemini-3.6-flash', timeout:2400}
  ];
  ...
  for (let index = 0; index < attempts.length; index++) {
    ...
    try {
      ...
      if (index < attempts.length - 1) {
        const retryAfter = Number(error.retryAfterSeconds || error.retryAfter || 0);
        const backoffMs = Number.isFinite(retryAfter) && retryAfter > 0
          ? Math.min(retryAfter * 1000, 2200)
          : Math.min(350 * (index + 1) * (index + 1), 1400);

...
function fallbackAnswer(message) {
  ...
  if (codes.length >= 2 && /clash|conflict|same time|timetable|schedule/i.test(q)) {
    const clashGroups = conflicts([...new Set(codes)]);
    if (clashGroups.length) {
      return clashGroups.map(group =>
        'Time clash: ' + group.courses.map(x => x.code + ' ' + x.name).join(' + ') + ' (' + group.slot.replace('|', ' ') + ').'
      ).join('\n');
    }
    return 'I do not see a same-slot clash among the course codes provided in the September 17, 2026 portal catalog.';
  }

  if (/\b(6|six)\b.*\bcours|mf301|mf302|mf303|mf401|mf402|mf404/.test(q)) {
    return [
  ...
  if (/what.*(field|area)|field.*interest|area.*interest|what.*speciali[sz]e|operations|credit|audit|risk|compliance|digital|agency|treasury|sme|agriculture|hr|public sector|non.?interest|deposit insurance|customer service|sustainable/i.test(q)) {
  ...
  if (/handout|publication|study pack|syllabus|examiner|mcp manual|materials|book|download/i.test(q)) {
    ...
    return 'The portal screenshot shows 75 paid examination publications across several price bands, plus separate downloadable examiner reports. Ask me for a title, course, or the price bands.';
  }
  ...
  if (/diploma|intermediate|chartered banker|801|802|803|804|805|806|807|808|809|810|811|812|813/i.test(q) &&
      /fee|price|cost|amount|pay/i.test(q)) {
    return 'Observed portal examples: ACIB Diploma (all four selected) ₦41,000; ACIB Intermediate (all four selected) ₦87,250; ACIB Chartered Banker example selection ₦135,000. These are screenshot cart observations, not a universal per-course tariff.';
  }

  if (/agency banking|cab|fee|price|cost|amount/i.test(q) && /cab\d{3}/i.test(q)) {
    return 'Observed portal example: CAB104 + CAB203 + CAB204 totalled ₦20,750. The portal timetable must be checked for the selected combination before treating a cart as valid.';
  }

  if (/e.?payment|ep\d{3}/i.test(q) && /fee|price|cost|amount/i.test(q)) {
    return 'Observed portal example: selecting the displayed E-Payments courses produced ₦271,000, but the portal simultaneously showed a timetable-clash warning, so this is not a valid combination and should not be treated as a universal fee.';
  }

  if (/annual subscription|life annual/i.test(q)) {
    return 'Two different amounts are present in the source package: the membership registration letter states ₦5,000 annual Student Member subscription due 1 January; a separate portal screen is labelled “Life Annual Subscription Payment” and shows ₦270,000. Do not treat those as the same membership charge.';
  }

  if (/exam fee|\bfee\b|how much.*exam|cost.*exam/.test(q)) {
    return 'The owner’s October 2026 MCP portal screenshot shows a total exam fee of ₦32,500.00. Payment-gateway charges may make the final amount higher; that checkout amount should not be presented as the CIBN base exam fee.';
  }

  if (/proctor|exam day|remote.*exam|online.*exam/.test(q)) {
    return 'The portal shows “REMOTE ONLINE PROCTORING PLATFORM” as the examination center for the October 2026 diet. I can help with readiness checks, but I will not invent vendor, disconnection, refund or misconduct rules that are not in the source material.';
  }

  if (/package|pricing|price|coach|mock|concierge|resit|b2b/i.test(q)) {

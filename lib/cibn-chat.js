...
const OFFICIAL_KNOWLEDGE = [
  ...
  'The October 2026 MCP timetable shows MF301 and MF302 in the Tuesday morning session, MF303 and MF402 in the Tuesday afternoon session, MF401 Wednesday morning, and MF404 Wednesday afternoon. The listed MCP papers are 1.5 hours each, so a two-paper 3-hour session is not itself a clash.',
...
function timetableAnswer(message) {
  ...
  return [
    ...
    'MCP: Tue Oct 6 morning MF301 + MF302 (1.5h each); Tue Oct 6 afternoon MF303 + MF402 (1.5h each); Wed Oct 7 morning MF401; Wed Oct 7 afternoon MF404.',
...
function fallbackAnswer(message) {
  ...
  if (/\b(6|six)\b.*\bcours|mf301|mf302|mf303|mf401|mf402|mf403|mf404/.test(q)) {
    return [
      ...
      'For October 2026, the published timetable shows MF404 in the Wednesday afternoon session. Confirm the elective actually selected in your CIBN portal before payment.'
    ].join('\n');
  }
...
async function cibnChat(req, res) {
  ...
  const mode = ['study', 'practice', 'human'].includes(req.body && req.body.mode) ? req.body.mode : 'study';
  ...
  if (mode === 'human') {
    try {
      ...
      return res.status(200).json({
        ok: true,
        mode: 'human',
        answer: 'Your request has been accepted by ABBA and queued for human ABBA. Continue through the human support channel below.',
        handoff
      });
    } catch (error) {
      ...
      return res.status(503).json({
        ok: false,
        mode: 'human',
        error: 'Human support queue is temporarily unavailable. Please use the published human support number: 0704 648 1828.'
      });
    }
  }
  ...
  for (const item of attempts) {
    try {
      ...
      return res.status(200).json({ok:true,answer:result.text,mode,sources:result.sources});
    } catch (error) {
      lastError = error;

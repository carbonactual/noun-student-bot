...
function safeNotify(text) {
  Promise.resolve(notifyAdmin(text)).catch((error) => console.error('Admin notification failed:', error.message));
}
function normalizeCourse(value) {
  return String(value || '').trim().toUpperCase().replace(/[^A-Z0-9_-]/g, '');
}
function parseCourses(text) {
  return [...new Set(String(text || '').split(',').map(normalizeCourse).filter(Boolean))].slice(0, MAX_COURSES);
}
...
client.on('message', async (msg) => {
  try {
    ...
    if (!student) {
      // Do not write an invalid NULL level into the students table.
      // The first level reply creates the actual student record.
    ...
    if (lower.startsWith('done ')) {
      ...
      const course = normalizeCourse(parts[1]);
      const stepNum = Number.parseInt(parts[2], 10);
    ...
    return msg.reply('Sorry — something went wrong processing that. Please try again in a moment.');
  }
});
...
async function handleAdminCommand(msg, number, text) {
  try {
    ...
    if (cmd === 'adddeadline') {
      const rest = text.split(/adddeadline\s+/i)[1] || '';
      const [meta, rawDate] = rest.split('|').map((s) => s.trim());
      const [level, rawCourse, ...titleParts] = meta.split(/\s+/);
      const course = normalizeCourse(rawCourse);
      const dueDate = parseDate(rawDate);
      if (!validLevel(level) || !course || !titleParts.length || !dueDate) {
        return msg.reply('Usage: admin adddeadline [100|200|300|400] [COURSE] [title] | [YYYY-MM-DD]');
      }
    ...
    if (cmd === 'creategroup') {
      const level = parts[2];
      const course = normalizeCourse(parts[3]);
      ...
      const result = await client.createGroup(`NOUN ${level}L ${course}`, participantIds);
    ...
    if (cmd === 'broadcast') {
      ...
      for (const num of matchingPhones) {
        try { await client.sendMessage(`${num}@c.us`, `📢 ${message}`); sent += 1; }
        catch (error) { console.error(`Broadcast failed for ${num}:`, error.message); }
      }
    ...
    return msg.reply('Admin command failed safely. Check the server logs.');
  }
}

// Daily reminders use Nigeria time explicitly, regardless of server location.
cron.schedule('0 8 * * *', async () => {
  try {
    const deadlines = await db.getAllDeadlines();
    const now = new Date();
    for (const deadline of deadlines) {
      const due = new Date(`${deadline.due_date}T23:59:59Z`);
      ...
      if ([3, 1].includes(daysLeft) && !remindedAt.includes(daysLeft)) {
        ...
        for (const num of matchingPhones) {
          try { await client.sendMessage(`${num}@c.us`, `⏰ Reminder: "${deadline.title}" for ${deadline.course} is due in ${daysLeft} day(s) (${deadline.due_date}).`); }
          catch (error) { console.error(`Reminder failed for ${num}:`, error.message); }
        }
        await db.markDeadlineReminded(deadline.id, [...remindedAt, daysLeft]);
      }
    }
    console.log(`Daily deadline check complete — ${deadlines.length} deadline(s) reviewed.`);
    safeNotify(`📊 Daily deadline check complete — ${deadlines.length} tracked deadline(s) reviewed.`);
  } catch (error) {
    console.error('Deadline cron error:', error);
    safeNotify(`🚨 Deadline cron failed: ${error.message}`);
  }

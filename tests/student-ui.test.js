...
const system = fs.readFileSync('assets/noun-system.css', 'utf8');
...
test('student space keeps the home simple and student-centered', () => {
  ...
  assert.ok(home.includes('Built on the ecosystem'));
  assert.ok(home.includes('The product is the entry. The foundations carry it.'));
...
test('student space uses rounded sans typography and accessible responsive behavior', () => {
  has('ui-rounded');
  has('system-ui');
  assert.ok(system.includes('--ca-gold'));
  assert.ok(system.includes('--ca-navy'));

...
const { DOMAINS, ALL_CATEGORIES, INTENTS, domainForCategory, classifyIntent, financialBoundary, buildStudentHome } = require('../lib/student-ecosystem');

test('student ecosystem covers academic, services, life, skills, work, commerce and finance', () => {
  for (const domain of ['academic','services','life','skills','work','commerce','finance','governance','representation','research']) {
    assert.ok(Array.isArray(DOMAINS[domain]));
    assert.ok(DOMAINS[domain].length > 0);
  }
  for (const category of ['nysc','sports','student_business','research_method','authorized_representative']) assert.ok(ALL_CATEGORIES.includes(category));
});
...
test('category maps to the correct student domain', () => {
  ...
  assert.equal(domainForCategory('research_method'), 'research');
});
...
test('student service requests and offers remain distinct intents', () => {
  assert.equal(classifyIntent('I need someone to photocopy my handout'), INTENTS.request_service);
...
test('finance remains discovery-only inside NOUN while execution belongs to I/O', () => {
  assert.deepEqual(financialBoundary('find student finance'), { allowed: true, action: 'find student finance', boundary: 'noun_intelligence' });
...
test('student home exposes all current operating surfaces without leaking private academic data', () => {

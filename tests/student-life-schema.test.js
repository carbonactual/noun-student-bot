...
test('student life schema contains events, groups, sports and excursions', () => {
  for (const table of ['student_events','student_event_participants','student_groups','student_group_memberships']) assert.match(sql, new RegExp(`create table if not exists ${table}\\b`));
...
test('skills and work schemas include skills acquisition and work pathways', () => {
  assert.match(sql, /student_skills/);
  assert.match(sql, /skill_opportunities/);
  assert.match(sql, /work_opportunities/);
  assert.match(sql, /skills text\[\] not null/);
  assert.match(sql, /apprenticeship/);
  for (const term of ['internship','employment','siwes','nysc']) assert.match(sql, new RegExp(term));
});

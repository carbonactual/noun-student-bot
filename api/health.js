...
module.exports = async function handler(req, res) {
  ...
  if (req.method === 'GET' && req.query?.check === 'gemini') {
    ...
    try {
      ...
      return res.status(ok?200:502).json({ok,service:'noun-student-bot',test:'gemini',model:'gemini-3.6-flash',configured:true,upstream_status:r.status,candidate_count:candidates.length,parts_count:parts.length,finish_reason:first?.finishReason||null,prompt_feedback:d?.promptFeedback?Object.keys(d.promptFeedback):[],response:answer.slice(0,80)});
  ...
  if (req.method === 'GET' && req.query?.check === 'integration') {
    try {
      ...
      if(!process.env.GEMINI_API_KEY) return res.status(503).json({ok:false,test:'integration',tenant:true,student_lookup:true,student_found:Boolean(student),knowledge_count:knowledge.facts.length,knowledge_confidence:knowledge.confidence,gemini_configured:false});
      ...
      const r=await fetch('https://generativelanguage.googleapis.com/v1beta/models/gemini-3.6-flash:generateContent',{method:'POST',headers:{'Content-Type':'application/json','x-goog-api-key':process.env.GEMINI_API_KEY},body:JSON.stringify({contents:[{role:'user',parts:[{text:prompt}]}]})});
      const d=await r.json(); const answer=d?.candidates?.[0]?.content?.parts?.map(p=>p?.text||'').join('').trim()||''; const ok=r.ok&&answer.length>0;
      return res.status(ok?200:502).json({ok,test:'integration',model:'gemini-3.6-flash',tenant:true,student_lookup:true,student_found:Boolean(student),knowledge_count:knowledge.facts.length,knowledge_confidence:knowledge.confidence,gemini_status:r.status,answer:answer.slice(0,500)});
  ...
  return res.status(ready ? 200 : 503).json({ok:ready,service:'noun-student-bot',version:'2.0.6',environment:process.env.VERCEL_ENV||'unknown',checks,timestamp:new Date().toISOString()});
};
